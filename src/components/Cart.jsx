...
const Cart = ({ cartItems, onUpdateQuantity, onRemoveItem }) => {
  ...
  return (
    <div className='bg-white p-6 rounded-lg shadow-lg transition-all duration-300 dark:bg-gray-900'>
      ...
      {cartItems.length === 0 ? (
        ...
        <>
            ...
            {cartItems.map(item => (
              <div key={item.id} className='flex flex-col md:flex-row items-center justify-between border-b pb-4'>
                <div className='flex items-center w-full md:w-1/2'>
                  <img src={`/assets/` + item.image} alt={item.title} className='w-20 h-20 object-cover rounded mr-4' />
                  <div>
                ...
                <div className='flex items-center my-2 md:my-0'>
                  <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className='px-3 py-1 border rounded-l 
                  disabled:opacity-50 transition-colors dark: border-gray-600' disabled={item.quantity <= 1}>
                    -
                  </button>
                  <span className='px-4 border-t border-b dark: border-gray-600'>{item.quantity}</span>
                  <button onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className='px-3 py-1 border 
                  rounded-l disabled:opacity-50 transition-colors dark: border-gray-600'>
                    +
                  </button>
                </div>
                <div className='text-right'>
                  ...
                  }} className='text-red-500 hover:underline dark: border-gray-600'>
                    Удалить
                  </button>
                </div>
              </div>
          ...
          <div className='mt-6 text-right'>
            ...
            {appliedDiscount > 0 && (
              <p className='text-xl font-semibold text-green-600 dark: text-gray-300'>
          ...
          <div className='mt-6'>
            ...
            <div className='flex flex-col md:flex-row gap-2'>
              ...
              />
              <button onClick={handleApplyDiscount} className='px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors'>
                Применить
              </button>
            </div>
          </div>
